program test_Rankine_ax
!-- Retrieve a 2-dim analytical vortex with a Rankine-type profile of Vt
  ...
  real, parameter :: max_diff_eps=1.0e-3
...
!-- calculate the maximum difference between analysis and retrieval
  call display_2valdiff_max( Vt_rht_t, VTtot_rt_t, undef=undef, cout=cvtmax, vout=dvtmax )
  call display_2valdiff_max( Ut_rht_t, VRtot_rt_t, undef=undef, cout=cvrmax, vout=dvrmax )
  call display_2valdiff_max( Vra_rt_t, Vratot_rt_t, undef=undef, cout=cvamax, vout=dvamax )

!-- test results
  if(max_diff_eps>dvtmax.and.max_diff_eps>dvrmax.and.max_diff_eps>dvamax)then
     call stdout( "Pass the test", "main", 0 )
  else
     call stdout( "The maximum difference is large", "main", 1 )
  end if
