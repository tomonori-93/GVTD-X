...
!! Calculate the coefficient matrix \(f_{kj}\)
  ...
  double precision, intent(out) :: fkj(nnk,size(theta))  !! Coefficient matrix
  logical, intent(in) :: undeflag(size(theta))  !! Undefined flag at each sampling point
...
!! Calculate \(b_k\) from \(f_{k,j}\) and \(V_d\)
  ...
  double precision, intent(in) :: deltaij(size(fkj,2))  !! The geometric factor
