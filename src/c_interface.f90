...
contains
  ...
  &          bind(C, name="c_Retrieval_velocity")
    ...
    real(c_double), value :: missing_value             !! undefined value for the original Doppler radar data
    ...
    real(c_double)        :: Vra_in(n,m,l)  !! input Doppler velocity [m/s]
    real(c_double)        :: VTtot(n,m,l)  !! retrieved total tangential wind [m s-1]
    real(c_double)        :: VRtot(n,m,l)  !! retrieved total radial wind [m s-1]
    real(c_double)        :: VRT0(n,m,l)  !! retrieved axisymmetric tangential wind [m s-1]
    real(c_double)        :: VDR0(n,m,l)  !! retrieved axisymmetric radial wind [m s-1]
    real(c_double)        :: Vra(n,m,l)      !! storm-relative input Doppler velocity [m s-1]
    ...
    real(c_double)        :: VRtot_Er(n,m,l)  !! earth-relative retrieved total radial wind [m s-1]
    real(c_double)        :: Uxtot_Er(n,m,l)  !! earth-relative retrieved total zonal wind [m s-1]
  ...
  &            n, m, nrot, ndiv, nrdiv, r, t, rh, td, rdiv, Vd, Vn, RadTC,  &
  ...
  &          bind(C, name="c_Retrieve_velocity_GVTDX")
    integer(c_int), value :: n, m, nrot, ndiv, nrdiv
    ...
    real(c_double)        :: rdiv(nrdiv)  !! radial coordinate on which Dc (staggered for Vd) is defined [m]
  ...
  &          bind(C, name="c_Retrieve_velocity_GVTD")
    ...
    real(c_double), value :: missing_value  !! missing value for Vd
    logical(c_bool), optional :: flag_datagap  !! Optimize the truncating wavenumber at each radius,  according to Lee et al. (2000) [Default: .false. (not optimize)]
    logical :: tmp_flag_datagap

    tmp_flag_datagap=.false.
    if(present(flag_datagap)) tmp_flag_datagap = flag_datagap  ! c_bool(LOGICAL(1)) to logical(LOGICAL(4))

    call Retrieve_velocity_GVTD( nasym, r, t, td, Vd, RadTC,  &
  &                              VT, VR, VT0, VR0, VTSn, VTCn, missing_value,  &
  &                              flag_datagap=tmp_flag_datagap )
